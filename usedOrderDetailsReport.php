...
class myPDF extends FPDF {
		public $title = "Usage Order Details Report";
        //Page header method
        function Header() {
        $this->SetFont('Arial','B',16);
        $w = $this->GetStringWidth($this->title)+175;
        $this->SetLineWidth(1);
...
$query1 = "SELECT
    ...
    $stmt1->execute();
    $result1 = $stmt1->fetch();
            $data1[] = array($result1['Used_OrderID'],
                    $result1['Used_By'],
                    $result1['Tool_Info'],
                    $result1['Used_OrderDate'],
                    $result1['Used_OrderNotes'],
                    $result1['Used_OrderStatus']);
            ...
            while ( $result = $stmt->fetch() ) {
                $data[] = array($result['ComponentSpecs'],
                    $result['Used_LineQuantity'],
                    $result['Used_LineCost'],
                    $result['Used_LineStatus'],
                    $result['Used_LineDate'],
                    $result['Used_LineSubTotal']);
            }
